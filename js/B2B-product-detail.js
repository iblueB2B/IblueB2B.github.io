...
const ProductDetail = {
            ...
            if (loadingState) {
                loadingState.innerHTML = `
                    <div style="text-align: center; padding: 50px 20px;">
                        ...
                        <p style="color: var(--gray-500); margin-top: 8px;">${error.message}</p>
    ...
    selectBulkTier(quantity) {
        ...
        event.currentTarget.style.background = 'var(--gray-100)';
    },

    // ============================================
    // CONTACT SUPPLIER - NEW FUNCTION
    // ============================================
    contactSupplier() {
        ...
        const inquiryData = {
            ...
            productPrice: this.productData.wholesale_price || this.productData.price,
            quantity: this.selectedQuantity,
            color: this.currentVariant?.color_name !== 'Default' ? this.currentVariant?.color_name : null,
            image: this.currentVariant?.image_url || this.productData.image_urls?.[0],
            supplierId: this.supplierId,
            supplierName: this.supplierProfile?.business_name || this.supplierProfile?.full_name,
            timestamp: new Date().toISOString()
        };
        
        // If user is logged in, add their info
        if (this.currentUser) {
            inquiryData.userId = this.currentUser.id;
        ...
        window.location.href = 'supplier-contact.html';
    },
    ...
    async toggleSave() {
        if (!this.currentUser) {
            // Save product ID to session for after login
    ...
    shareProduct() {
        if (navigator.share) {
            navigator.share({
                ...
                text: `Check out this product on iBlue B2B: ${this.productData?.title || ''}`,
