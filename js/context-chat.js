// ============================================
// UNIFIED CONTEXT CHAT - INQUIRIES/QUOTES/ORDERS
// ============================================
...
let ContextChat = {
    ...
    currentContext: null, // { type: 'inquiry'|'quote'|'order', data: {} }
    conversation: null,
    messages: [],
    ...
    async loadInquiryContext(inquiryId) {
        try {
            ...
            if (isSupplier) {
                ...
                this.otherParticipant = data.profiles;
            } else {
                // User is buyer - find the supplier they're chatting with
                // For now, take the first supplier who's responded
    ...
    async loadQuoteContext(quoteId) {
        try {
            ...
            if (isSupplier) {
                // User is supplier - other participant is buyer
                this.otherParticipant = data.inquiry_requests.profiles;
            } else {
                // User is buyer - other participant is supplier
                this.otherParticipant = data.suppliers?.profiles;
            }
    ...
    async loadOrCreateConversation() {
        try {
            const contextField = `${this.currentContext.type}_id`;
            ...
            const { data, error } = await sb
                ...
                .insert({
                    ...
                    [contextField]: parseInt(this.currentContext.id),
    ...
    async sendContextIntroMessage() {
        ...
        if (this.currentContext.type === 'inquiry') {
            ...
            message = `💰 Discussing quote #${quote.quote_number} for UGX ${this.formatNumber(quote.total_amount)}`;
        ...
        try {
            ...
            const { data: message, error } = await sb
                ...
                .insert({
                    ...
                    [`${this.currentContext.type}_id`]: parseInt(this.currentContext.id),
    ...
    renderContextBar() {
        ...
        if (!bar || !this.currentContext) return;
        ...
        if (type === 'inquiry') {
            ...
            details = `
                ...
                <p>
                    <span><i class="fas fa-boxes"></i> ${data.inquiry_items?.length || 0} items</span>
                    <span><i class="fas fa-calendar"></i> Expires: ${this.formatDate(data.expires_at)}</span>
            ...
            actions = `
                ...
                ${!this.currentUserProfile?.is_supplier ? `
                    <button class="context-btn" onclick="ContextChat.editContext()">
            ...
            icon = 'fa-file-invoice';
            const items = data.supplier_quote_items || [];
            const total = items.reduce((sum, item) => sum + (item.total_price || 0), 0);
            
            details = `
                <h2>Quote from ${this.escapeHtml(data.suppliers?.business_name || 'Supplier')}</h2>
                <p>
            ...
            if (this.currentUserProfile?.is_supplier) {
                actions = `
                    <button class="context-btn" onclick="ContextChat.viewContextDetails()">
            ...
            details = `
                <h2>Order with ${this.escapeHtml(data.suppliers?.business_name || data.buyer?.business_name || '')}</h2>
                <p>
                    <span><i class="fas fa-tag"></i> UGX ${this.formatNumber(data.total_amount)}</span>
            ...
            if (this.currentUserProfile?.is_supplier) {
                ...
                actions = `
                    ...
                    ${['pending', 'confirmed'].includes(data.status) ? `
                        <button class="context-btn danger" onclick="ContextChat.requestCancellation()">
        ...
        if (this.currentContext.type === 'inquiry') {
            ...
            if (isSupplier) {
                actions = [
                    ...
                    { icon: 'fa-truck', text: 'Update Tracking', action: 'showTrackingModal' }
                ];
            } else {
                actions = [
                    { icon: 'fa-eye', text: 'Track', action: 'viewContextDetails' },
                    { icon: 'fa-check-circle', text: 'Confirm Delivery', action: 'showConfirmDelivery', class: 'primary' },
    ...
    askQuestion() {
        document.getElementById('messageInput').value = 'I have a question about this inquiry. ';
    ...
    showAcceptQuote() {
        ...
        summary.innerHTML = `
            ...
            <div class="summary-row">
                ...
                <span>${this.formatPaymentTerms(quote.payment_terms?.[0])}</span>
            </div>
            <div class="summary-row">
    ...
    showCounterOffer() {
        ...
        const items = this.currentContext.data.supplier_quote_items || [];
    ...
    updateCounterSummary() {
        ...
        document.getElementById('counterOfferSummary').innerHTML = `
            <div class="summary-row">
    ...
    async sendCounterOffer() {
        ...
        document.getElementById('messageInput').value = counterMessage;
    ...
    reportIssue() {
        document.getElementById('messageInput').value = '⚠️ I need to report an issue with this order: ';
        document.getElementById('messageInput').focus();
    },
    
    requestCancellation() {
        document.getElementById('messageInput').value = '❌ I would like to request cancellation of this order. ';
        document.getElementById('messageInput').focus();
    },
    ...
    setupRealtimeSubscriptions() {
        ...
        const contextChannel = sb
            ...
            .on('postgres_changes',
                { event: 'UPDATE', schema: 'public', table: `${this.currentContext.type}s`, filter: `id=eq.${this.currentContext.id}` },
                async () => {
                    // Reload context data
                    if (this.currentContext.type === 'inquiry') {
    ...
    getContextItems() {
        ...
        if (this.currentContext.type === 'inquiry') {
            ...
            return data.supplier_quote_items || [];
        } else if (this.currentContext.type === 'order') {
    ...
    setupEventListeners() {
        ...
        document.getElementById('confirmAcceptBtn')?.addEventListener('click', () => this.acceptQuote());
        document.getElementById('sendCounterBtn')?.addEventListener('click', () => this.sendCounterOffer());
        document.getElementById('sendRevisionBtn')?.addEventListener('click', () => this.sendRevisionRequest());
        document.getElementById('confirmDeliveryBtn')?.addEventListener('click', () => this.confirmDelivery());
        document.getElementById('saveTrackingBtn')?.addEventListener('click', () => this.updateTracking());
    ...
    showMenu() {
        ...
        menuItems.push(
            { icon: 'fa-flag', text: 'Report', action: 'reportIssue', class: 'danger' }
        );
